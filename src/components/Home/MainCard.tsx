...
export default function MainCard() {
  return (
    <section className="flex flex-col gap-y-4 bg-biscay p-10 sm:p-28">
      ...
      <div className="grid grid-cols-1 items-center justify-between gap-9 sm:grid-cols-2">
        {content.Home.ProgramUnit.ProgramUnitCard.map((val, index) => (
          <SingleMainCard key={index} icon={val.logo} title={val.headerOfTitle} desc={val.info} />
        ))}
      </div>
    </section>
  );
